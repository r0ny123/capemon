...
{
	if (g_osverinfo.dwMajorVersion == 10 && g_osverinfo.dwBuildNumber == 19044)
	{
#ifndef _WIN64
		return ((ULONG_PTR)(mod)+0xC7E0);
#else
		return ((ULONG_PTR)(mod)+0xD7A0);
#endif
	}
	else
	{
...
#ifndef _WIN64
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x8b\x40\x10\x57\x50\x8b\x30\x8b\x4e\x50", 10);
		}
		else {
			addr = find_string_in_bounds(start, end, "\xff\x75\x1c\x8b\x46\x04\xff\x75\x18\x8b\x40\x10\xff\x75\x14\x8b\x08\x57\xff\x75\x08\x50\xff\x51\x50\x8b\xd8", 27);
		}
#else
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x48\x8b\x47\x08\x48\x8b\x78\x20\x48\x8b\x07\x48\x8b\x98\xa0\x00\x00\x00", 18);
		}
		else {
			addr = find_string_in_bounds(start, end, "\x45\x8b\xcd\x4c\x8b\xc7\x48\x8b\xd6\x41\xff\x92\xa0\x00\x00\x00", 16);
		}
#endif
		if (addr == NULL)
			return 0;
		...
		while (addr > addr2 - 256) {
			if (!memcmp(addr, "\x90\x90\x90\x90", 4) || !memcmp(addr, "\xcc\xcc\xcc\xcc", 4))
				return (ULONG_PTR)addr + 4;
			addr--;
		}
	}
...
ULONG_PTR get_wmi_execmethod_addr(HMODULE mod)
{
	...
	{
#ifndef _WIN64
		return ((ULONG_PTR)(mod)+0x88B10);
#else
		return ((ULONG_PTR)(mod)+0xA30C0);
#endif
	}
	else
	{
		...
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x8b\x40\x10\x53\x57\x50\x8b\x30\x8b\x4e\x60", 11);
		}
		else {
			addr = find_string_in_bounds(start, end, "\xff\x75\x18\xff\x75\x14\x57\xff\x75\x08\x50\xff\x51\x60\x8b\xd8", 16);
		}
#else
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x48\x8b\x47\x08\x48\x8b\x78\x20\x48\x8b\x07\x48\x8b\x98\xc0\x00\x00\x00", 18);
		}
		else {
			addr = find_string_in_bounds(start, end, "\x48\x89\x44\x24\x20\x45\x8b\xcc\x4c\x8b\xc7\x48\x8b\xd6\x41\xff\x92\xc0\x00\x00\x00\x8b\xd8", 23);
		}
#endif
		if (addr == NULL)
			return 0;
...
ULONG_PTR get_wmi_execqueryasync_addr(HMODULE mod)
{
	...
	{
#ifndef _WIN64
		return ((ULONG_PTR)(mod)+0x9FB0);
#else
		return ((ULONG_PTR)(mod)+0xD920);
#endif
	}
	else
	{
		...
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x8b\x40\x10\x57\x50\x8b\x30\x8b\x4e\x54", 10);
		}
		else {
			addr = find_string_in_bounds(start, end, "\xff\x75\x14\x8b\x08\x53\xff\x75\x08\x50\xff\x51\x54\x8b\xf8", 15);
		}
#else
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x48\x8b\x47\x08\x48\x8b\x78\x20\x48\x8b\x07\x48\x8b\x98\xa8\x00\x00\x00", 18);
		}
		else {
			addr = find_string_in_bounds(start, end, "\x45\x8b\xcd\x4c\x8b\xc7\x48\x8b\xd6\x41\xff\x92\xa8\x00\x00\x00\x8b\xd8", 18);
		}
#endif
		if (addr == NULL)
			return 0;
...
ULONG_PTR get_wmi_execmethodasync_addr(HMODULE mod)
{
	...
	{
#ifndef _WIN64
		return ((ULONG_PTR)(mod)+0x88C50);
#else
		return ((ULONG_PTR)(mod)+0xA32C0);
#endif
	}
	else
	{
		...
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x53\x8b\x40\x10\x57\x50\x8b\x30\x8b\x4e\x64", 11);
		}
		else {
			addr = find_string_in_bounds(start, end, "\xff\x75\x18\x8b\x08\xff\x75\x14\x53\xff\x75\x08\x50\xff\x51\x64\x8b\xf8", 18);
		}
#else
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x48\x8b\x47\x08\x48\x8b\x78\x20\x48\x8b\x07\x48\x8b\x98\xc8\x00\x00\x00", 18);
		}
		else {
			addr = find_string_in_bounds(start, end, "\x45\x8b\xcd\x4c\x8b\xc7\x48\x8b\xd6\x41\xff\x92\xc8\x00\x00\x00\x8b\xd8", 18);
		}
#endif
		if (addr == NULL)
			return 0;
...
ULONG_PTR get_wmi_getobject_addr(HMODULE mod)
{
	...
	{
#ifndef _WIN64
		return ((ULONG_PTR)(mod)+0xC940);
#else
		return ((ULONG_PTR)(mod)+0x2ED50);
#endif
	}
	else
	{
		...
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x53\x8b\x30\x50\x8b\x4e\x18", 7);
		}
		else {
			addr = find_string_in_bounds(start, end, "\x51\xff\x75\x14\xff\x75\x10\x53\x50\xff\x52\x18\x8b\xf8", 14);
			if (addr == NULL) // windows xp
				addr = find_string_in_bounds(start, end, "\x51\xff\x75\x14\xff\x75\x10\x57\x50\xff\x52\x18\x89\x45\x0c", 15);
		}
#else
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x48\x8b\x47\x08\x48\x8b\x78\x20\x48\x8b\x07\x48\x8b\x58\x30", 15);
		}
		else {
			addr = find_string_in_bounds(start, end, "\x4c\x8b\xcd\x45\x8b\xc4\x48\x8b\xd7\x41\xff\x52\x30\x8b\xd8", 15);
		}
#endif
		if (addr == NULL)
			return 0;
...
ULONG_PTR get_wmi_getobjectasync_addr(HMODULE mod)
{
	...
	{
#ifndef _WIN64
		return ((ULONG_PTR)(mod)+0x88F00);
#else
		return ((ULONG_PTR)(mod)+0xA36D0);
#endif
	}
	else
	{
		...
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x57\x8b\x30\x50\x8b\x4e\x1c", 7);
		}
		else {
			addr = find_string_in_bounds(start, end, "\x8b\x08\x53\x50\xff\x51\x1c\x8b\xf8", 9);
		}
#else
		if (g_osverinfo.dwMajorVersion == 10) {
			// on windows 10 the code is different due to CFG checking
			addr = find_string_in_bounds(start, end, "\x48\x8b\x47\x08\x48\x8b\x78\x20\x48\x8b\x07\x48\x8b\x58\x38", 15);
		}
		else {
			addr = find_string_in_bounds(start, end, "\x4d\x8b\xcc\x45\x8b\xc5\x48\x8b\xd7\xff\x50\x38\x8b\xd8", 14);
		}
#endif
		if (addr == NULL)
			return 0;
